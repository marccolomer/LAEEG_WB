...
age = 12; % IT CAN BE 9 OR 12 AND SHOULD BE CHANGED DEPENDING ON WHAT AGE YOU ARE ANALYZING
...
phase_adj_mat = zeros(pos_f_max-pos_f_min+1, chan_num, chan_num);
...
for trial = 1:size(data_fft.tfcomplex_data,3) % Loop across trials 
    for t = 1:length(times2save) % Loop across time  
        ...
        for freq=pos_f_min:pos_f_max % Position from 6Hz to 9Hz in the frequency vector
            ...
            [phase_adj_mat(it_fr,:,:)] = phase_connectivity_acrosstime_github(complex_reshaped);
            ...
            [thresh_out, connect_degree_out, connect_data_out, connect_binary_out] = ...
                connectivity_degree_thres_github(squeeze(phase_adj_mat(it_fr,:,:)), thresh);

            thres_ispc_time(t,it_fr,trial) = thresh_out;
        end
        phase_avg_freq(t,:,:,trial) = squeeze(mean(phase_adj_mat,1)); % mean across frequencies
    end
end
ispc_connectivity_time = squeeze(mean(phase_avg_freq,4)); % average across trials

%% save data
save_data2=[save_location, subject_label, '_wholebrain_time_', freq_range, '.mat'];
