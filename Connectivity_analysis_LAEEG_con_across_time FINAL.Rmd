...
library(lmerTest)
#library(mediation)
#path_mac_new <- '/Users/marccolomercanyelles/Box Sync/EEG/LAEEG/plots/'
path_mac_new <- '/Users/marccolomer/Box/EEG/LAEEG/plots/2022/'
...
# Whole Brain
df_wide_phl_9m <- read.csv("T_thres_acrosstime_cl_9m_ispc.csv", sep=",")
df_phl_9m <- gather(df_wide_phl_9m, Participant, Threshold, -Time, - Condition, -Age)
df_wide_phl_12m <- read.csv("T_thres_acrosstime_cl_12m_ispc.csv", sep=",")
...
thres_wb <- select(data_wb_all,c(Threshold,Participant, Time, Condition))%>%
  #mutate(len_name = nchar(Participant))%>%
  #mutate(Participant_2 = paste(substr(Participant,1,8), "", substr(Participant,9,len_name), sep = ""))%>%
  mutate(Condition_2 = ifelse(Condition == "Cane", "cane", "grasp"))
#thres_wb$Participant <- thres_wb$Participant_2
...
d_behavior <- read.csv("912m_Beh_Marc_121820_HCedit.csv", sep=",")%>%
...
## REMOVE THIS!
Anova <- ezANOVA(data=sum_all_stats_fil, dv = mean_stats, wid = Participant, within = .(Condition, TimeGroup, ChannelPair), between = .(Age), type = 3)
show(Anova)
...
```{r thresholdTheta, echo=FALSE}
  ...
  p1 
  ggsave('wb_thres_acrosstime_timewin_ispc_cl15_c2.png', plot = last_plot(), device = 'png', scale = 0.75, width = 6, height = 4, path = path_mac_new)
    
     
  p1<- ggplot(sum_phl, aes(x=TimeGroup, y=mean_group, group = Age_Condition, linetype=Age, color=Condition)) + geom_line()+geom_point()+geom_errorbar(aes(ymin = mean_group-sderr_group, ymax = mean_group+sderr_group), width = .2, position = position_dodge(0.05)) + ylab("ISPC Whole Brain") + xlab("Time Window") + ylim(0.49, 0.55)
  #+ ylim(0.53, 0.575)
    # + facet_grid(Age ~ .)
...
sum_all_stats_wide<- sum_all_stats_t2 %>%
  #unite("ChanP_Cond", c(Condition,ChannelPair), remove = TRUE)%>%
  #spread(ChanP_Cond,mean_stats)
...
d_con_beh <- merge(sum_all_stats_wide,d_behavior, by = "rowid")%>%
  ...
  #mutate(GraspLat = Competence_include_OneTrial_Grasp/1000)%>%
  #mutate(CaneLat = Competence_include_OneTrial_Cane/1000)
...
#lm_oc <- lm(Connectivity ~ GraspLat*Age*Condition, data = d_con_beh_oc)
#summary(lm_oc)

#lm_pc <- lm(Connectivity ~ GraspLat*Age*Condition, data = d_con_beh_pc)
#summary(lm_pc)

#lm_fc <- lm(Connectivity ~ GraspLat*Age*Condition, data = d_con_beh_fc)
#summary(lm_fc)
...
#m.0 <- lmer(Connectivity ~ Condition + (1 | Participant) , data=d_con_beh_oc, REML = FALSE)  # build
...
#m.0 <- lmer(Connectivity ~ Condition + (1 | Participant) , data=d_con_beh_fc, REML = FALSE)  # build
...
p_bar 
ggsave('Bar_ISPCnormWB_PC_groupcompetence_-1000-500_cl15_c2.png', plot = last_plot(), device = 'png', scale = 0.75, width = 7, height = 4, path = path_mac_new)

## 1. VIOLIN PLOT WITH GRASP LATENCY: Generate competence groups based on grasp latency
d_con_beh_noNaN <- d_con_beh[!is.na(d_con_beh$GraspLat),]%>%
  mutate(group_competence = ifelse(GraspLat > median_comp, "Slow", "Fast"))

# Plot connectivity based on grasp_latency group
p_violin <- ggplot(d_con_beh_noNaN, aes(x=group_competence, y=Connectivity, fill=Condition)) + 
  geom_violin(position=position_dodge(0.85),width = 1, trim=TRUE)+
  geom_boxplot(position=position_dodge(0.85),width = 0.15)+
  xlab("Group Competence") + ylab("ISPC anticipation") + labs(fill = "Condition") + facet_grid(.~ChanPair)
p_violin #+ ggsave('Violin_ISPCnormWB_groupcompetence_-1000-500.png', plot = last_plot(), device = 'png', scale = 0.8, width = 11, height = 9, path = path_mac_new)
...
#Anov_oc_aov = aov(Connectivity ~ group_competence*Condition + Error(Participant/Condition)
#                  +(group_competence), data=d_con_beh_oc,)
#summary(Anov_oc_aov)
...
## 3. CANE LATENCY
median_comp <- median(d_con_beh$CaneLat[which(!is.na(d_con_beh$CaneLat))])
d_con_beh_noNaN <- d_con_beh[!is.na(d_con_beh$CaneLat),]%>%
  mutate(group_competence = ifelse(CaneLat > median_comp, "Slow", "Fast"))
...
p_violin <- ggplot(d_con_beh_noNaN, aes(x=group_competence, y=Connectivity, fill=Condition)) + 
  ...
  geom_boxplot(position=position_dodge(0.85),width = 0.15)+
  xlab("Group Competence") + ylab("ISPC anticipation") + labs(fill = "Condition") + facet_grid(.~ChanPair)
p_violin #+ ggsave('Violin_ISPCnormWB_groupcompetence(Cane)_-1000-500.png', plot = last_plot(), device = 'png', scale = 0.8, width = 11, height = 9, path = path_mac_new)

## SEPARATE DATA BASED ON CHANNEL PAIR
d_con_beh_oc <- d_con_beh_noNaN %>%
  filter(ChanPair == "OC")
d_con_beh_fc <- d_con_beh_noNaN %>%
  filter(ChanPair == "FC")
...
# STATISTICS FOR EACH CHANNEL PAIR (ANOVA BASED ON GROUP COMPETENCE)
Anov_oc <- ezANOVA(data=d_con_beh_oc, dv = Connectivity, wid = Participant, within = .(Condition), between = .(group_competence, Age), type = 3)  # build
show(Anov_oc)

Anov_fc <- ezANOVA(data=d_con_beh_fc, dv = Connectivity, wid = Participant, within = .(Condition), between = .(group_competence, Age), type = 3)  # build
show(Anov_fc)

Anov_pc <- ezANOVA(data=d_con_beh_pc, dv = Connectivity, wid = Participant, within = .(Condition), between = .(group_competence, Age), type = 3)  # build
show(Anov_pc)
...
## WHOLE BRAIN CONNECTIVITY AND BEHAVIOR
...
p_data <- ggplot(d_con_beh_noNaN, aes(x=GraspLat, y=Connectivity, shape=Condition, colour=Condition, fill=Condition)) + geom_point(shape=1) +     # Use hollow circles
  geom_smooth(method=lm) + ylab("WHOLE BRAIN ISPC: -2000 to 1000ms") + xlab("Grasp Latency (s)")
p_data #+ ggsave('wholebrainVSGraspLat_acrosstime_Allwin_Noage_new.png', plot = last_plot(), device = 'png', scale = 0.7, width = 10, height = 7, path = path_mac_new)
...
## Post-hoc test on the interaction Age * Condition (anticipatory period)
d_con_beh_noNaN_9 <- filter(d_con_beh_noNaN,Age == "9m")
d_con_beh_noNaN_12 <- filter(d_con_beh_noNaN,Age == "12m")

A_9 <- ezANOVA(data=d_con_beh_noNaN_9, dv = Connectivity, wid = Participant, within = .(Condition), type = 3)  # build
show(A_9)
A_12 <- ezANOVA(data=d_con_beh_noNaN_12, dv = Connectivity, wid = Participant, within = .(Condition), type = 3)  # build
show(A_12)
...
p_data #+ ggsave('wholebrainVSCaneLat_acrosstime_Allwin_age_new.png', plot = last_plot(), device = 'png', scale = 0.7, width = 10, height = 7, path = path_mac_new)

#d_con_beh_grasp <- d_con_beh_noNaN %>%
#  filter(Condition == "grasp") ## remove subjects with NaN
...
## WHOLE BRAIN CORRELATES WITH CO_normWB?

```{r thetanormWB, echo=FALSE}

## SUMMARIZE DATA
lbls <- c('-1500:-1000', '-1000:-500', '-500:0', 
          '0:500', '500:1000')
...
sum_all_stats <- data_all_with_wb%>%
  ...
  group_by(TimeGroup = cut(Time, 
               breaks = seq(min_time, max_time, by = 500), labels = lbls), Condition, Age, Participant, ChannelPair)%>%
#  group_by(Condition, Age, Participant, ChannelPair)%>%
  #summarise(mean_stats = mean(connectivity_norm), mean_wb = mean(thres_wb)) # NORMALIZED
  summarise(mean_stats = mean(Connectivity), mean_wb = mean(thres_wb)) # NO NORMALIZED
...
p_data + ggsave('ChanPairvsWB_acrosstime_grasp_-1000-500.png', plot = last_plot(), device = 'png', scale = 0.7, width = 13, height = 6, path = path_mac_new)

#linearMod_grasp <- lm(mean_stats ~ mean_wb*Age*ChannelPair, data=d_con_fil)  # build
...
#m.0 <- lmer(mean_stats ~ ChannelPair + Age + Age*ChannelPair + (1 | Participant), data=d_con_fil, REML = FALSE)  # build
...
p_data + ggsave('ChanPairvsWB_acrosstime_cane_-1000-500.png', plot = last_plot(), device = 'png', scale = 0.7, width = 13, height = 6, path = path_mac_new)

#linearMod_cane <- lm(mean_stats ~ mean_wb*Age*ChannelPair, data=d_con_fil)  # build
...
## GRASP AND CANE TOGETHER
d_con_fil <- filter(sum_all_stats_t2)%>%  
#  filter(Participant != "connect_overtime_LA_EEG_Pilot09_CTS_MTH_chanpair_9m.mat")
  filter(Age == "12m")
p_data <- ggplot(d_con_fil, aes(x=mean_wb, y=mean_stats, shape=Condition, colour=Condition, fill=Condition)) +
  geom_point(shape=1) +     # Use hollow circles
  geom_smooth(method=lm) + facet_grid(. ~ ChannelPair) + ylab("ISPC") + xlab("ISPC Whole Brain ")
p_data + ggsave('ChanPairvsWB_acrosstime_12m_-1000-500.png', plot = last_plot(), device = 'png', scale = 0.7, width = 13, height = 6, path = path_mac_new)


m.0 <- lmer(mean_stats ~ ChannelPair + (1 | Participant), data=d_con_fil, REML = FALSE)  # build

# ADD THE NEXT MORE "OBVIOUS" VARIABLE
m.1_0 <- update(m.0, .~. + Condition) 
m.1_1 <- update(m.1_0, .~. + Condition*ChannelPair)

# FINALLY WE ADD WB
m.2_0 <- update(m.1_1, .~. + mean_wb)
m.2_1 <- update(m.2_0, .~. + mean_wb*ChannelPair)
m.2_2 <- update(m.2_0, .~. + mean_wb*Condition)
m.2_3 <- update(m.2_0, .~. + mean_wb*ChannelPair*Condition)

anova(m.0,m.1_0, m.1_1, m.2_0,m.2_1, m.2_2, m.2_3) # 2_0 is not sign
...
lm_oc <- lm(mean_stats ~ Condition*mean_wb, data=d_con_fil_oc)  # build
summary(lm_oc)
lm_fc <- lm(mean_stats ~ Condition*mean_wb, data=d_con_fil_fc)  # build
summary(lm_fc)
lm_pc <- lm(mean_stats ~ Condition*mean_wb, data=d_con_fil_pc)  # build
summary(lm_pc)


d_con_fil_oc_g <- filter(d_con_fil_oc, Condition == "grasp")
d_con_fil_oc_c <- filter(d_con_fil_oc, Condition == "cane")
lm_g <- lm(mean_stats ~ Age*mean_wb, data=d_con_fil_oc_g)  # build
summary(lm_g)
lm_c <- lm(mean_stats ~ Age*mean_wb, data=d_con_fil_oc_c)  # build
summary(lm_c)
...
d_behavior<- rowid_to_column(d_behavior)
#  pivot_longer(
#  cols = Cane.Mu.1000500:Grasp.Palpha.5001000,
#  names_to = "Condition.Cluster.TimeWin",
#  values_to = "Power"
#) %>%
#  separate(Condition.Cluster.TimeWin,c("Condition", "Cluster", "TimeWin"))
...
p_data + ggsave('ChanPair_normWbvsPower_acrosstime_grasp_-1000-500.png', plot = last_plot(), device = 'png', scale = 0.8, width = 11, height = 9, path = path_mac_new)
...
p_data + ggsave('ChanPair_normWbvsPower_acrosstime_cane_-1000-500.png', plot = last_plot(), device = 'png', scale = 0.8, width = 11, height = 9, path = path_mac_new)

linearMod_cane <- lm(Cane.Mu.1000500 ~ Connectivity*Age*ChanPair, data=d_con_fil)  # build
summary(linearMod_cane)


#### CONNECTIVITY DURING ANTICIPATION: DOES IT PREDICT CONNECTIVITY DURING ACTION????
## 1. GRASP 
d_con_fil <- filter(d_con_power,Condition == "grasp")
p_data <- ggplot(d_con_fil, aes(x=Connectivity, y=Grasp.Mu.0500, shape=Age, colour=Age, fill=Age)) +
  geom_point(shape=1) +     # Use hollow circles
  geom_smooth(method=lm) + facet_grid(. ~ ChanPair) + ylab("Mu Power Grasp") + xlab("ISPC across time")
p_data + ggsave('connectAnticipation_vs_PowerAction_acrosstime_grasp.png', plot = last_plot(), device = 'png', scale = 0.8, width = 11, height = 9, path = path_mac_new)

linearMod_grasp <- lm(Grasp.Mu.0500 ~ Connectivity*Age*ChanPair, data=d_con_fil)  # build
summary(linearMod_grasp)

## 2. CANE
d_con_fil <- filter(d_con_power,Condition == "cane")
p_data <- ggplot(d_con_fil, aes(x=Connectivity, y=Cane.Mu.0500, shape=Condition, colour=Age, fill=Age)) +
  geom_point(shape=1) +     # Use hollow circles
  geom_smooth(method=lm) + facet_grid(. ~ ChanPair)  + ylab("Mu Power Cane") + xlab("ISPC across time")
p_data + ggsave('connectAnticipation_vs_PowerAction_acrosstime_cane.png', plot = last_plot(), device = 'png', scale = 0.8, width = 11, height = 9, path = path_mac_new)

linearMod_cane <- lm(Cane.Mu.0500 ~ Connectivity*Age*ChanPair, data=d_con_fil)  # build
summary(linearMod_cane)

```


## CORRELATE WB CONNECTIVITY AND POWER

```{r thetanormWB, echo=FALSE}

df_wide_phl_FC <- read.csv("T_FC_ispc_time_9m_noNorm.csv", sep=",")
df_phl_FC_9m <- gather(df_wide_phl_FC, Participant, Connectivity, -Time, - Condition, -Age, -ChannelPair)
df_wide_phl_PC <- read.csv("T_PC_ispc_time_9m_noNorm.csv", sep=",")
df_phl_PC_9m <- gather(df_wide_phl_PC, Participant, Connectivity, -Time, - Condition, -Age, -ChannelPair)
df_wide_phl_OC <- read.csv("T_OC_ispc_time_9m_noNorm.csv", sep=",")
df_phl_OC_9m <- gather(df_wide_phl_OC, Participant, Connectivity, -Time, - Condition, -Age, -ChannelPair)

df_wide_phl_FC <- read.csv("T_FC_ispc_time_12m_noNorm.csv", sep=",")
df_phl_FC_12m <- gather(df_wide_phl_FC, Participant, Connectivity, -Time, - Condition, -Age, -ChannelPair)
df_wide_phl_PC <- read.csv("T_PC_ispc_time_12m_noNorm.csv", sep=",")
df_phl_PC_12m <- gather(df_wide_phl_PC, Participant, Connectivity, -Time, - Condition, -Age, -ChannelPair)
df_wide_phl_OC <- read.csv("T_OC_ispc_time_12m_noNorm.csv", sep=",")
df_phl_OC_12m <- gather(df_wide_phl_OC, Participant, Connectivity, -Time, - Condition, -Age, -ChannelPair)
...
data_all_OC <- data_all%>%
  filter(ChannelPair == "OC" & Time>-2000 & Time<1000)
data_all_PC <- data_all%>%
  filter(ChannelPair == "PC" & Time>-2000 & Time<1000)
data_all_FC <- data_all%>%
  filter(ChannelPair == "FC" & Time>-2000 & Time<1000)

thres_wb <- data_wb_all$Threshold
data_all_OC <- cbind(data_all_OC, thres_wb)
data_all_PC <- cbind(data_all_PC, thres_wb)
data_all_FC <- cbind(data_all_FC, thres_wb)

data_all_with_wb <- data_all_OC%>%
  mutate(connectivity_norm = thres_wb) ## whole brain
...
p_data <- ggplot(d_con_fil, aes(x=Connectivity, y=Grasp.Mu.0500, shape=Age, colour=Age, fill=Age)) +
  geom_point(shape=1) +     # Use hollow circles
  geom_smooth(method=lm) + ylab("Mu Power Grasp") + xlab("ISPC Whole Brain across time")
p_data + ggsave('wbvsPower_acrosstime_grasp_0500.png', plot = last_plot(), device = 'png', scale = 0.8, width = 11, height = 9, path = path_mac_new)

linearMod_grasp <- lm(Grasp.Mu.0500 ~ Connectivity*Age, data=d_con_fil)  # build
summary(linearMod_grasp)
...
p_data <- ggplot(d_con_fil, aes(x=Connectivity, y=Cane.Mu.0500, shape=Condition, colour=Age, fill=Age)) +
  geom_point(shape=1) +     # Use hollow circles
  geom_smooth(method=lm) + ylab("Mu Power Cane") + xlab("ISPC Whole Brain across time")
p_data + ggsave('wbvsPower_acrosstime_cane_0500.png', plot = last_plot(), device = 'png', scale = 0.8, width = 11, height = 9, path = path_mac_new)

linearMod_cane <- lm(Cane.Mu.0500 ~ Connectivity*Age, data=d_con_fil)  # build
summary(linearMod_cane)
