...
subnum_wb=dir([data_location_wb '*' 'CTS_MTH_wholebrain_time.mat' '*']);

sub_list_wb={subnum_wb.name}; 

n_subjects = length(sub_list_wb);

data_init = load([pwd '/LAEEG_9m/wb_output/LA_EEG_Pilot02_CTS_MTH_wholebrain_time.mat']);
...
for co =1:length(condition_name) 
    ...
    subnum_wb=dir([data_location_wb '*' [condition_name{co} '_wholebrain_time.mat'] '*']);    
    ...
    for s=1:n_subjects
       ...
       [thres_NOcl_ispc(s,:,co)] = connectivity_cluster_thres(squeeze(phaseconnectivity_ispc(s,:,:,:)), thres_formula);
    ...
    save_data1=[save_location, 'wholebrain_all_acrosstime_', age, '_', condition_name{co}, '.mat'];               
    save (save_data1, 'phaseconnectivity_ispc', 'connectivity_cluster_ispc', 'sub_list_wb', '-v7.3');
end

save_data2=[save_location, 'threshold_all_acrosstime_', thres_formula, age, '.mat'];    
